...
<body>
	<div class="panel admin-panel">
		...
		<form method="post"
			action="chinaHistoryHistory!getAllByType?pagenow="
			id="listform">
			<div class="padding border-bottom">
				<ul class="search" style="padding-left: 10px;">
					<li><a class="button border-main icon-plus-square-o"
						href="page/history/history_history_add.jsp"> 添加内容</a></li>
					<li>朝代：</li>
		...
		<table class="table table-hover text-center">
			<!-- 查询结果 -->
			<c:if test="${request.china_History_Histories!=null }">
				<tr>
				...
				<c:forEach items="${china_History_Histories}" var="s">
					<tr>
						...
						<td><div class="button-group">
								<a class="button border-main"
									href="chinaHistoryHistory!update?flag=update&id=${s.id }"><span
				...
				<c:if test="${request.show != null }">
					<tr>
						<td colspan="8"><div class="pagelist">
								...
								<form
									action="chinaHistoryHistory!getAllByType?title=&type=${request.type}"
									method="post">
									<a class="button border-main"
										href="chinaHistoryHistory!getAllByType?title=&type=${request.type}&pagenow=1">
										第一页</a>&nbsp; <a class="button border-main"
										href="chinaHistoryHistory!getAllByType?title=&type=${request.type}&pagenow=${pagenow - 1}">上一页</a>&nbsp;
									<a class="button border-main"
										href="chinaHistoryHistory!getAllByType?title=&type=${request.type}&pagenow=${pagenow + 1}">下一页</a>&nbsp;
									<a class="button border-main"
										href="chinaHistoryHistory!getAllByType?title=&type=${request.type}&pagenow=${pagecount}">最后一页</a>
	...
	<script type="text/javascript">
		...
		function del(id) {
			if (confirm("您确定要删除吗?")) {
				window.location.href = ("chinaHistoryHistory!delete?type=上古&title=&pagenow=&id="+id);
			}
		}
	</script>
